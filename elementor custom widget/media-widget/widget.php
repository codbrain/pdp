...
class Media_Widget extends \Elementor\Widget_Base
{
	...
	{
		return __('Media block', 'media-image');
	}
...
?>
		<div id="block<?php echo $unique_id; ?>">
			<img id="block-img<?php echo $unique_id; ?>" src="<?php echo $image_url; ?>">
			<video id="block-video<?php echo $unique_id; ?>" src="<?php echo $video_url; ?>" style="display: none;" muted loop playsinline></video>
			<div id="overlay<?php echo $unique_id; ?>">
		...
		<style>
			...
			#block-img<?php echo $unique_id; ?> {
				width: 100%;
				height: 100%;
				object-fit: cover;
			}

			#block-video<?php echo $unique_id; ?> {
				width: 100%;
				height: 100%;
				object-fit: cover;
			}

			<?php if ('yes' === $show_textarea) : ?>#overlay<?php echo $unique_id; ?> {
				position: absolute;
				bottom: 0;
				left: 0;
				right: 0;
		...
		<script>
			document.getElementById("block<?php echo $unique_id; ?>").addEventListener("mouseenter", showVideo);
			document.getElementById("block<?php echo $unique_id; ?>").addEventListener("mouseleave", hideVideo);

			function showVideo() {
				// Get the video element
				let video = document.getElementById("block-video<?php echo $unique_id; ?>");

				// Get the image element
				let image = document.getElementById("block-img<?php echo $unique_id; ?>");

				// Get the overlay element
				let overlay = document.getElementById("overlay<?php echo $unique_id; ?>");
